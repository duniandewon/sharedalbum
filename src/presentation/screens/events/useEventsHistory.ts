...
export function useEventsHistory(
  getEventsByHostIdUseCase: GetEventByHostIdUseCase = new GetEventByHostIdUseCase()
) {
  ...
  const navigateToEvent = (eventShareId: string, isActive: boolean) => {
    navigate(
      isActive ? `/event/${eventShareId}` : `/event/${eventShareId}/album`
    );
  };
